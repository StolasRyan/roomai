const PROJECT_PREFIX = 'roomai_project_';
...
router.post('api/projects/save', async({request, user})=>{
    try{
        ...
        if(!userPuter) return jsonError(401, 'Authification failed');
        ...
        if(!project?.id || !project?.sourceImage) return jsonError(400, 'Project not found');
         ...
         if(!userId) return jsonError(401, 'Authification failed');
...
router.get('api/projects/list', async({user})=>{
    try{
        const userPuter =user.puter;
        if(!userPuter) return jsonError(401, 'Authfication failed');

        const userId = await getUserId(userPuter);
        if(!userId) return jsonError(401, 'Authfication failed');
